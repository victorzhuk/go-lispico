...
- [x] 1.1 Red adversarial tests: tight allocation loop; macro-amplified
...
- [x] 2.3 Reduction flush: tree-walker `pollCancel` and VM `pollCancel`
...
- [x] 2.4 Charge reductions in macro expansion (per step) and compiler emit
  (per instruction), against the same evaluation ledger.
- [x] 2.5 Charge one reduction per GoFunc dispatch at the two apply sites
  (`core/eval.go` `apply`, `core/vm/vm.go` `apply`); verify no other dispatch
  path exists.
- [x] 2.6 Allocation charges with the fixed size table: VM
...
- [x] 2.7 Reader bridge: reader accumulates node/byte counts during parse;
...
- [x] 2.8 Document the size table in ADR 0011 (values + conservativeness
  rationale + determinism requirement).

## 3. Integration

- [x] 3.1 Crossval: same adversarial programs under the same limits on both
...
- [x] 4.1 `openspec validate --strict engine-reduction-and-allocation-metering`.
