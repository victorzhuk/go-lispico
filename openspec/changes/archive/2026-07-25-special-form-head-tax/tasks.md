...
- [ ] 1.1 Paired gold-set capture before any edit: `GOMAXPROCS=2`,
...
- [ ] 1.2 Record allocs/op per fixture. That is the deterministic signal this
      change is judged on; timing here is too noisy for a few percent to mean
      anything.

## 2. The fix

- [ ] 2.1 In `resolveHead` (`core/eval.go`), short-circuit a head naming a
...
- [ ] 2.2 Place it AFTER the Lisp-2 function-cell lookup and BEFORE
...
- [ ] 2.3 Comment the WHY, not the what: that special forms are dispatched
      by `evalList` and are never values, so evaluating one only builds an
      error to discard.

## 3. Prove it

- [ ] 3.1 `TestEval_MacroExpand_UndefinedHead` green — a genuinely undefined
      head must still report unresolved. The short-circuit fires only for
      names in `e.forms`, which an undefined symbol is not.
- [ ] 3.2 Add or extend a test covering a Lisp-2 function-cell macro bound
...
- [ ] 3.3 Crossval `TestVMVsTreeWalker` 218 passed — it covers CL and
      Clojure dialects including function-cell rebinds.

## 4. Measure

- [ ] 4.1 Paired capture after, same parameters. Report allocs/op deltas per
      fixture.
- [ ] 4.2 Expected shape, and the check that the fix hit what it claims:
      ...
      accepting the win.
- [ ] 4.3 Report whether `guard-nil` and `rule-load` reach parity with the
...
- [ ] 5.3 `go test ./internal/goldset/ -count=1` — 27 passed.
