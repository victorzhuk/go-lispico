...
- [ ] 1.1 Profile `Goldset/guard-nil` under both execution modes and identify
...
- [ ] 1.2 Do the same for `Goldset/kw-lookup` (bytes −9.04%, latency −20.00%)
...
- [ ] 1.3 State whether the three cells share one cause or have three. The
...
- [ ] 2.1 Settle what ADR 0008's startup tier bounds. Its stated rule is
...
- [ ] 2.2 Make `internal/perfgate/perfgate.go` match the decision, and record
      ...
      documents do.
- [ ] 2.3 Cover the decision with a test in `internal/perfgate/perfgate_test.go`
...
- [ ] 3.1 Reduce the VM's allocation on the cells 1.3 says are fixable, and
...
- [ ] 3.2 For any cell 1.3 says is not fixable, amend ADR 0008's threshold for
      ...
      steps.
- [ ] 3.3 CHANGELOG `[Unreleased]`: what moved, on which cells, and by how
...
- [ ] 5.1 `openspec validate --strict` on this change.
- [ ] 5.2 `go build ./... && go test ./... -count=1` and
