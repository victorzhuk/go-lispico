...
- [ ] 2.1 Same-chunk return: on `OpReturn`/`OpTailCall` return where the
...
- [ ] 2.2 Same-chunk call: when the callee closure's chunk equals the
      loaded chunk, skip re-deriving code/truthy/caps on frame entry.
- [ ] 2.3 Re-audit throw/handler unwind and terminal Reset against the
...
- [ ] 3.1 Depth-counter elision: plain-field depth when the counter is the
...
- [ ] 3.2 Reset split: per-call arm vs full reset. Enumerate every `Reset`
      ...
      keep the full reset.
- [ ] 3.3 Conditional `bumpRunGenIfWrapped`: skip the defer when
