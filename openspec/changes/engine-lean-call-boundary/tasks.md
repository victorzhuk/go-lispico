...
- [ ] 2.1 Engine fast-flag: bytecode evaluator + no engine meter + no
...
- [ ] 2.2 Root-env atomic snapshot: replace the per-call `e.mu.RLock` root
...
- [ ] 3.1 Extend the call cache hit path to a versioned snapshot read
...
- [ ] 3.2 Prove the read is coherent: value+version read under the same
...
- [ ] 4.1 Single recover-defer for the fast path covering VM release,
      panic→`NewPanicError`, and stats bump; no StartEval/FinishEval, no
      `nowFunc` (callbacks are off by the flag's definition).
- [ ] 4.2 Per-engine VM slot: atomic CAS claim/release with pool fallback;
...
- [ ] 4.3 Undefined-name path, arity errors, Lisp-2 fallback resolution:
