...
- [ ] 1.1 Red tests, tree-walker: `(loop [] (try body (catch e nil)))` under a
  ...
  (characterization).
- [ ] 1.2 Red tests, VM: resource-limit raised inside a called closure /
...
- [ ] 1.3 Characterization: explicit `(throw ...)` of arbitrary values,
  including error-looking strings, remains catchable on both evaluators.

## 2. Implementation

- [ ] 2.1 `core.IsTerminalEvalError(err) bool`: `errors.Is` on
  `context.Canceled` / `context.DeadlineExceeded`; `errors.As` on
  `*LispicoError` with `Code == CodeResourceLimit`.
- [ ] 2.2 `evalTry` uses the classifier in place of its inline
  resource-limit-only check.
- [ ] 2.3 VM: filter before `vm.throw` at `OpCall`, `OpTailCall`, native-op
  dispatch, and the throw-coercion path; terminal errors take the direct
  return route.

## 3. Integration

- [ ] 3.1 Crossval: same adversarial programs, same limits, both evaluators
  → same terminal error class, never caught.
- [ ] 3.2 `go test ./... -race`; `GOLDSET_MODE=vm` goldset gate non-increasing.

## 4. Verification

- [ ] 4.1 `openspec validate --strict eval-noncatchable-terminal-errors`.
