...
- [ ] 1.1 Interleaved baseline (one session, `-count=10`): fib, Rule,
      goldset both modes, `-benchmem`; `resolveGlobalValue` CPU share on
      fib. Untouched-row control.
- [ ] 1.2 Write the semantic pin FIRST: crossval tests for (a) callee
...
- [ ] 2.1 Fused opcode carrying {site index, argc}; compiler emits it for
...
- [ ] 2.2 Dispatch: read the site's versioned snapshot (the existing
...
- [ ] 2.3 Guard audit: enumerate every mutation that can change what the
...
- [ ] 2.4 `Chunk.Validate` cases for the new opcode(s): site index, argc
      vs stack depth, non-fall-through rules.

## 3. Verify

- [ ] 3.1 Crossval `TestVMVsTreeWalker` + the 1.2 pins green.
- [ ] 3.2 Hot-reload + UnloadPlugin scenarios from the resolved-globals
