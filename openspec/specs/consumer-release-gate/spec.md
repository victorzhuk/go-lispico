...
## Purpose
Decide, from evidence the repository owns, whether a go-lispico release may be
authorized on performance. Release CI measures a committed gold set under both
execution modes and judges each cell against per-tier thresholds recorded in ADR
0008, so a regression blocks the cut without a consumer checkout, a revision pin,
or a cross-repo dependency. The gate states what it cannot conclude rather than
guessing: evidence it cannot pair, a comparison across a change of runner, and a
cell it has no threshold for are each reported as such instead of being resolved
into a pass.
