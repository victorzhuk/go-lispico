...
- Forms the VM does not compile (currently a `defmacro` nested inside a larger form, a `def`/`defn` inside a lexical scope, and `unquote-splicing`) must return a clean error and defer to the Evaluator, never panic — the whole enclosing top-level form is tree-walked before any of its bytecode runs. The two evaluators must agree on results, including the runtime type bound by `catch`.
