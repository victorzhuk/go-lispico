...
| Evaluator conversion entry buffer | 24 bytes once per builder-to-trie conversion + 64 per key/value pair |

There is one construction header, `MeterCollectionHeaderBytes` (24), and it prices both pieces of construction storage the table names: the storage the reader's map builder obtains and the entry buffer the evaluator's builder-to-trie conversion obtains. It is not the `MeterHashMapHeaderBytes` (32) output term of `HashMapShallowBytes`, and the two are separate charges for separate storage rather than one charge counted twice. A construction header belongs to storage a single call obtains; the reader's own work buffers — `nodePlan`, `formPlan`, `entryPlan`, admitted through `growthPlan.admit` — are read-scoped, live across every form of the read, and carry no header at all: they are priced per slot on a doubling schedule, the pricing the last bullet below states. The persistent-map node row is an evaluator charge: `hamtNodeBytes`, applied through `hamtSizeBytes` on the `Assoc`/`Dissoc` path. No reader path builds a trie node, so no reader path charges it. The same row prices the nodes a builder-to-trie conversion publishes. That conversion is a per-value charge settled by the first update on that value: the first `Assoc`/`Dissoc` against a builder-form map converts its storage and is charged the nodes it publishes, and every later update on the same value finds the conversion already published and charges only its own path. The published trie is the memo; its storage is charged to the evaluation that built it and is not re-charged as retained capacity when a host keeps the value past that evaluation. The footprint that buys is real: the published trie stays reachable for as long as the builder-form map is, on top of `large.m`, which stays authoritative. While any map derived from it is alive the incremental cost is near zero through structural sharing; the exposed case is a long-lived builder-form receiver whose derived maps have all been dropped, where the trie used to be garbage and now is not. `Set` is the only thing that clears it. The conversion obtains two pieces of storage on the one call, and the table owns both: the persistent-map nodes, priced by this row at their construction site, and the entry buffer the conversion's deterministic ordering fills, priced by the conversion entry buffer row. That row reads 24 as one slice header, the same reading the list/vector header bullet below takes, plus 64 per pair for the buffer's payload; the allocator rounds that payload up to a size class, so the charge does not under-count the storage.
