...
- `Env.Evaluator` now reads under the owner's lock, matching writes to the same field. The residual gap is `NewEnv`, which copies the parent's evaluator field without taking the parent's lock when constructing a child: a host calling `RootEnv().Child()` while a plugin `Init` is concurrently changing the root's evaluator through the registration view can still race on that read. Tracked, not yet closed.
