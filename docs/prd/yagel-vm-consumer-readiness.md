...
go-lispico owns the gate corpus as a **Gold set** — rule-shaped fixtures with hand-derived golden results plus benchmark cells, committed in `internal/goldset` and modeled on embedder Rule workloads, independent of any consumer. A go-lispico release job runs the gold set under both execution modes and performs a **Paired release run**, self-contained — no consumer checkout. Passing the gate authorizes YAGEL to add `WithBytecode()` directly. It does not change go-lispico's global Engine default.
...
- go-lispico owns the gate's **Gold set** — fixtures, hand-derived goldens, benchmark cells, and checked-in **Hot-cell tiers** — independent of any consumer. YAGEL keeps its own shipped Rule corpus, Behavior goldens, and scheduler/bus checks on its side; they inform gold-set evolution but the gate does not consume them directly.
...
- go-lispico release CI runs the committed **Gold set** under both execution modes, self-contained in the candidate checkout. No consumer checkout, no revision pin, no cross-repo secret; the corpus and its tier assignments are repo-owned (ADR 0008).
...
- The **Gold set** ships with twelve committed cells covering dispatch, closure state, error handling, keyword lookups, macros, loops, collection folds, string building, and rule loading, with committed tier assignments. Scale-envelope cells (baseline/knee/boundary per data dimension) and concurrent cells are not yet authored (ADR 0008).
