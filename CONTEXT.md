...
- YAGEL owns its corpus and benchmarks, while go-lispico release CI runs them against the candidate at YAGEL's `gold` ref — the blessed-release pointer YAGEL advances; no revision pin is recorded in go-lispico.
...
- Gate ownership was ambiguous between go-lispico, YAGEL, and the benchmark lab — resolved: YAGEL owns the live corpus and benchmarks; go-lispico release CI runs YAGEL's `gold` ref against the candidate.
...
- A pinned YAGEL revision could drift from the live consumer like a copied fixture — resolved: the gate tracks YAGEL's `gold` ref instead of a recorded pin, so YAGEL owns pointer movement and the gate always runs against the currently blessed release.
