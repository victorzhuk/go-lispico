...
- YAGEL owns the gate corpus as a **Gold set** — rule-shaped fixtures with golden expected results plus benchmark cells, exported from its shipped rules and committed in go-lispico; the release gate runs it self-contained, with no consumer checkout or revision pin.
...
- Gate ownership was ambiguous between go-lispico, YAGEL, and the benchmark lab — resolved: YAGEL owns the **Gold set** content, exported from its shipped rules; go-lispico release CI runs the committed gold set against the candidate.
...
- A pinned or checked-out consumer could couple the public release job to the private YAGEL repo — resolved: the gate runs a committed **Gold set** instead, accepting drift bounded by YAGEL's deliberate export/refresh; no cross-repo secret and no private output in public logs.
