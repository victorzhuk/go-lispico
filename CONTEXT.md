...
- YAGEL owns its corpus and benchmarks, while go-lispico release CI runs them against the candidate at a pinned YAGEL revision, re-pinned to the latest YAGEL release at each go-lispico release cut.
...
- Gate ownership was ambiguous between go-lispico, YAGEL, and the benchmark lab — resolved: YAGEL owns the live corpus and benchmarks; go-lispico release CI runs a pinned YAGEL revision against the candidate.
...
- The pinned YAGEL revision could drift from the live consumer like a copied fixture — resolved: each go-lispico release cut re-pins to the latest YAGEL release, bounding staleness to one cycle.
