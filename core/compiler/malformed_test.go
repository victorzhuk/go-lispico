...
func TestCompiler_MalformedForms(t *testing.T) {
	...
	cases := []struct {
		...
		{"let non-vector bindings", mlist("let", core.Int{V: 5}, core.Int{V: 1})},
		{"let odd binding count", mlist("let", mvec("x"), core.Int{V: 1})},
		{"let* non-vector bindings", mlist("let*", core.Int{V: 5}, core.Int{V: 1})},
		{"let* odd binding count", mlist("let*", mvec("x"), core.Int{V: 1})},
		{"recur outside loop", mlist("recur", core.Int{V: 1})},
